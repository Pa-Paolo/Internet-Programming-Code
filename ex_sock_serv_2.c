...
char buf[100];
	...
	printf("Client is %s on port %d\n",inet_ntoa(cli.sin_addr),port); //inet_ntoa=ritorna la notazione puntata dell'indirizzo in BIG ENDIAN
...
struct sockaddr_in cli;		/* Endpoint address of the client  */
    ...
    *  Metto a zero tutta la struttura sin. E' meglio farlo in modo che la struttura sia completamente pulita*/
	...
	if (bind(s, (struct sockaddr *)&sin, sizeof(sin)) < 0) { //Il secondo argomento è l'indirizzo alla quale voglio connettermi (Passo il puntatore alla struttura by referance)
	...
	while (1) {
	   ...
	   /* now fork a child to handle the connection */ //CREO un child per servire il client che ha fatto scattare l'accept.
	   if (fork() == 0) { //Il child (0) chiude il socket, il numero di child dipende dai client connected, se ho 5 client connessi
           //allora avrò 5 children plus the parent (6 processi)
           close(s); //CHIUDERE NEL CHILD NON SIGNIFICA CHIUDERLO NEL PARENT, nel kernel c'è una tabella che tiene conto di quanti programmi
           //stanno usando un certo oggetto quindi quando faccio la fork il contatore aumenta e chiudendolo rimane comunque qualcuno che lo sta usando.
           //Quando chiudo decremento il contatore di 1, se il contatore arriva a zero distrugge l'ggetto altrimenti lo lascia li.
		do_server(cs, cli, n);
	   	}
	   close(cs);
	   }
...
/*
 * Per provare il server facciamo nel terminale:
 *
 */
