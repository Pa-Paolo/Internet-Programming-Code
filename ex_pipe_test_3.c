...
int sleepf=0;
	...
	if((pid1 = fork()) == 0) {
		...
		close (pfd[0]); //Posso chiuderlo perchè l'output del pipe ora è associato allo standard input
		 ...
		 * Il cat process riceve come input lo standard input che però è stato sostituito con l'output della pipe.
